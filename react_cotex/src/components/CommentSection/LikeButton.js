...
const LikeButton = ({ commentId, handleLikeToggle, isLiked, likesCount, updateLikesCount }) => {
  const handleLikeClick = async () => {
    await handleLikeToggle(commentId);
    // Update like count after API call
    updateLikesCount();
  };

  return (
    <div className={`like-container-1 ${isLiked ? 'liked' : ''}`} onClick={handleLikeClick}>
